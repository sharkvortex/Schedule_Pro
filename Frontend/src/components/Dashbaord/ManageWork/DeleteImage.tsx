...
function DeleteImage({ fileId, onConfirm, onCancel , loading }: DeleteImageProps) {
  return (
      ...
      <div className="bg-white rounded-xl p-6 shadow-lg w-80 text-center space-y-4">
        ...
        <div className="flex justify-center gap-4 mt-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-md bg-red-500 hover:bg-red-600 text-white font-medium ${loading ? "hover:cursor-not-allowed" : "hover:cursor-pointer"} `}
          >
           {loading ? <div className="flex items-center justify-center px-4"><Loading size={10}/></div> : "ลบ"} 
          </button>
        </div>
      </div>
    </div>
