...
function Subject() {
  ...
  const { fetchWork, workData ,subjectLoading } = useWorkSubject();
      ...
      <div className="w-full">
        ...
        <div className="p-8">
          <div className="flex  justify-between items-center  mb-6">
            ...
            <div>
              <select  value={selectedSubjectId} onChange={(e) => setSelectedSubjectId(e.target.value)} className="bg-white text-gray-500 outline-none border border-gray-300/50  text-sm rounded-lg cursor-pointer  block w-full p-2.5">
                <option value="desc">ล่าสุด</option>
                <option value="asc">เก่าสุด</option>
              </select>
            </div>
          </div>
          {subjectLoading && <div className="w-full min-h-[200px] flex items-center justify-center"><Loading/></div>}
          {/* dataWork */}
